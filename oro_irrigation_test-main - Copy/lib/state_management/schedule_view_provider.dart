...
class ScheduleViewProvider extends ChangeNotifier {
  ...
  void fetchDataAfterDelay(deviceId, userId, controllerId, BuildContext context) {
    isLoading = true;
      ...
      return Future.delayed(Duration.zero, () {
        ...
        return Future.delayed(const Duration(seconds: 8), () {
          if (!scheduleGotFromMqtt) {
            return getUserSequencePriority(userId, controllerId, messageFromHttp);
          }
        }).whenComplete(() {
          isLoading = false;
          ...
          print(scheduleList);
          for(var i = 0; i < scheduleList['Status'].length; i++) {
            statusListFetched.add(getStatusInfo(scheduleList['Status'][i].toString()).statusString);
          }
          ...
          scheduleCount.forEach((category, count) {
            scheduleCountList.add(count);
          });
          // scheduleCount = scheduleList[]
          // if(scheduleList.)
          // if(isFetchingCompleted) {
          //   showSnackBar(context, 'Data fetched from mqtt');
          // } else {
          //   showSnackBar(context, 'Data fetched from http');
          // }
          notifyListeners();
        });
      });
