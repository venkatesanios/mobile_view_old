...
class IrrigationProgramProvider extends ChangeNotifier {
  ...
  void addNextSequence({int? serialNumber, zoneSno, indexToInsert}) {
    ...
    // // // print("zoneSno ==> ${zoneSno}");
    // // // print("indexToInsert ==> ${indexToInsert}");
    _irrigationLine!.sequence.insert(
        indexToInsert+1,
        {
          "sNo": "${serialNumber == 0 ? serialNumberCreation : serialNumber}.${_irrigationLine!.sequence.length+1}",
          "id": 'SEQ${serialNumber == 0 ? serialNumberCreation : serialNumber}.${_irrigationLine!.sequence.length+1}',
          "name": 'Sequence ${serialNumber == 0 ? serialNumberCreation : serialNumber}.${_irrigationLine!.sequence.length+1}',
          "selected": false,
          "location": '',
          "valve": [],
          "mainValve": [],
  ...
  void addValvesInSequence({valves, int? serialNumber, int? sNo, required int lineIndex, required int sequenceIndex, required bool isMainValve}) {
    for (var i = 0; i < valves.length; i++) {
      if(checkValveContainment2(valves: valves, sequenceIndex: sequenceIndex, i: i, isMainValve: isMainValve)){
        if(isMainValve) {
